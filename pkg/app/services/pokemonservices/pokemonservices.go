...
func TranslateDescription(description string, translationType string) (output pokemonmodels.TranslateResponseVM, myErr error) {
	resultOfTranslation := pokemonmodels.TranslateResponseVM{}

	jsonData := pokemonmodels.CreateFunDescription{Text: description}
	jsonValue, _ := json.Marshal(jsonData)
	u := bytes.NewReader(jsonValue)
	baseUrlFun := "https://api.funtranslations.com/translate"
	postUrlFun := fmt.Sprintf("%s/%s", baseUrlFun, translationType)

	request, err := http.NewRequest("POST", postUrlFun, u)
	...
	if response.StatusCode != http.StatusOK {
		return output, errors.New(fmt.Sprintf("error fetching data, status - %d", response.StatusCode))
	}
	...
	err = json.Unmarshal(body, &resultOfTranslation)

	if err != nil {
		return output, err
	}

	return resultOfTranslation, nil
}
